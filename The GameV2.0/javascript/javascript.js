...
if (munition > 100) { //geeft een alert als je meer dan 100 invuld
	alert('Ben je Rambo ofzo? Dit kun je helemaal niet dragen!');
} else if (munition >= 16 && munition < 100) { //de else if controleert deze conditie als de eerste niet klopt.
...
if (lopen <= 32){ //als er minder dan 32 word ingevoerd word ie fout gerekent. als er blauw word ingevoerd dat word de else uitgevoerd.
	alert("Dat is fout!");
} else {
	alert("Goed gedaan!");
	var zwaailicht = prompt("Welke kleur hebben de zwaailichten?");
}

if (zwaailicht != "blauw") { //als het niet blauw is voert ie de alert uit.
	alert("Hoe kun jij nou agent zijn. Je bent ontslagen. Of je hebt een spelfout gemaakt.");
} else if (zwaailicht == "blauw") { //checkt nog een keer of er blauw is geschreven. Zo ja, dan worden de twee lines uitgevoerd.
